""" This script combines two very hot topics from Deep Seek
   MC Hyper connections and Engrams. This sub-word transformer implementation is
   simple to run focuses on the core concepts of this research paper . It is not as light-weight as
   the character level transformer and would require 10 GB of VRAM to train. In order to conserve VRAM I have also
   included support for BF16 , reducing the memory required by half."""
...
import os
import matplotlib
import tiktoken  # NEW: Sub-word tokenizer
...
BLOCK_SIZE = 128
LEARNING_RATE = 3e-4
MAX_ITERS = 18000  # Full training run, for better results increase this to 50K
EVAL_INTERVAL = 200
...
# Initial Warmup Setting
WARMUP_STEPS = 600

# mHC Specifics
N_STREAMS = 4
EMBED_DIM = 256  # Total Model Width = 256 * 4 = 1024
N_LAYERS = 8  # Increased from 4 to 8
SINKHORN_ITERS = 20

# Engram Specifics
ENGRAM_ACTIVE = True
# As per paper apply engrams to early layers,
# "Early + Middle" Strategy for 6 Layers (Indices 0-5)
# Layer 0: Static Reconstruction
# Layer 3: Contextual Gating
ENGRAM_LAYERS = [0, 4]
# Larger Prime Numbers for Sub-word vocab (~50k)
ENGRAM_VOCAB_SIZES = [65521, 65521]
ENGRAM_EMBED_DIM = 128
MAX_NGRAM = 3
MODEL_SAVE_PATH = './sub_word_model/engrams_hyper_connection_v3.pt'
...
# --- Visualization ---
def visualize_engram_gates(model, tokenizer, text_segment, layer_idx=0):
    model.eval()

    # Encode with BPE
    enc = tokenizer.encode(text_segment)
    ...
    with torch.no_grad():
        ...
        avg_gate = torch.stack(gates_list, dim=2).mean(dim=2).squeeze(-1).squeeze(0)

    # Decode tokens for labels
    tokens = [tokenizer.decode([i.item()]) for i in input_ids[0]]
    activations = avg_gate.cpu().numpy()

    # Just show the actual text length
    valid_len = len(enc)
    # Clip to context if longer
    valid_len = min(valid_len, BLOCK_SIZE)
...
if __name__ == "__main__":
    def get_data(filename="./train_data/text8_full.txt"):
        if os.path.exists(filename):
            with open(filename, 'r', encoding='utf-8') as f:
                return f.read()

        print("Downloading Text8 (100MB Uncompressed)...")
        # Full Text8
    ...
    # 1. Setup Tokenizer (GPT-2 BPE)
    print("Initializing BPE Tokenizer (tiktoken: gpt2)...")
    ...
    # 2. Load Data
    text = get_data()
    # Encode all data (this might take a moment for 100MB)
    print("Encoding Data...")
    ...
    # 3. Init Model
    print(f"Initializing Engram-mHC Model (L={N_LAYERS}, Streams={N_STREAMS}, BF16)...")
    os.makedirs(os.path.dirname(MODEL_SAVE_PATH), exist_ok=True)
    if os.path.exists(MODEL_SAVE_PATH):
        try:
            model = torch.load(MODEL_SAVE_PATH, weights_only=False).to(DEVICE)
            print("Model Loaded! Setting WARMUP_STEPS = 0") # When loading a pre-existing model no warmup required.
            WARMUP_STEPS = 0
        except Exception as e:
            print('Model not loaded')
    else:
        model = MHCEngramLM(vocab_size).to(DEVICE)
        print("Model Not Loaded. Starting fresh.")
        WARMUP_STEPS = 600

    optimizer = torch.optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY)

    # 4. Training Loop with BF16
    print("Starting BF16 Training...")
    ...
    out_ids = model.generate(ctx, 300)[0].tolist()
    print(enc.decode(out_ids))

    # 6. Visualization
    print("Visualizing Gates...")
    vis_text = "The French Revolution was a period of radical political and societal change in France."
    visualize_engram_gates(model, enc, vis_text, layer_idx=4)
